...
class ClipAnnotator(QMainWindow):
    def __init__(self):
        ...
        self.adjust_video = False     # False=offset moves skeleton, True=offset moves video clip
    ...
    def _build_ui(self):
        ...
        self.adj_video_cb = QCheckBox("Adjust Video (offset moves clip, not skeleton)")
        self.adj_video_cb.setChecked(False)
        self.adj_video_cb.stateChanged.connect(lambda s: setattr(self, "adjust_video", s == Qt.Checked) or self._show_frame())
        ff.addRow(self.adj_video_cb)
    ...
    def _on_act_sel(self, row):
        ...
        e = ov.get("end", a["end"])
        # In adjust_video mode, shift clip range by total offset
        if self.adjust_video:
            total_off = self.scene_offset + self._get_effective_act_offset(row)
            s += total_off
            e += total_off
            # Clamp to valid video range
            s = max(0, s)
            e = max(s, min(self.vtotal - 1, e)) if self.vtotal > 0 else max(s, e)
    ...
    def _on_act_off(self, val):
        ...
        if self.adjust_video:
            self._on_act_sel(self.cur_act)  # recompute clip range
        else:
            self._show_frame()
        self._auto_save()
    ...
    def _on_scene_off(self, val):
        ...
        if self.adjust_video and self.cur_act >= 0:
            self._on_act_sel(self.cur_act)  # recompute clip range
        else:
            self._show_frame()
        self._auto_save()
    ...
    def _show_frame(self):
        ...
        if need_skel:
            ...
            # In adjust_video mode, offset moves the clip (video), not the skeleton
            skel_off = 0 if self.adjust_video else total_off
    ...
    def _export(self, all_actions, single_cam=False):
        ...
        for ai in indices:
            ...
            # In adjust_video mode, shift clip range; skeleton offset = 0
            if self.adjust_video:
                sf += total_off
                ef += total_off
                sf = max(0, sf)
            ...
            # points csv
            if self.pts3d is not None:
                pi_s = v2p(sf, self.vfps, self.pfps, self.pts3d.shape[0], export_skel_off)
                pi_e = v2p(ef, self.vfps, self.pfps, self.pts3d.shape[0], export_skel_off)
                        ...
                        if self.pts3d is not None and self.calibs:
                            ...
                            for cal_cam, (intr, extr) in self.calibs.items():
                                pidx = v2p(fi, fps, self.pfps,
                                           self.pts3d.shape[0], export_skel_off)
                                pts = self.pts3d[pidx]
                                if self.pts3d_valid is not None and self.pts3d_valid[pidx]:
                                    proj = project_pts(pts, intr, extr,
                                                       self.flip[0], self.flip[1], self.flip[2])
